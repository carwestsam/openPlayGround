...
/***/ function(module, exports, __webpack_require__) {
	...
	var Validation = React.createClass({
	  ...
	  validate: function () {
	    ...
	    var ret = "";
	    if (typeof source == "undefined" || source.trim().length <= 0) {
	      ret = "Required";
	    }
	    if (this.props.type === "link" && !validateLink(source)) {
	      ret = "Invalid URL";
	    }
	    if (ret.length > 0) hasError = true;
	    return ret;
	  },
	  render: function () {
	...
	var ResourceListItem = React.createClass({
	  ...
	  onChange: function (key) {
	    return function (e) {
	      this.props.handleChange(key, e.target.value, this.props.idx)(e);
	    }.bind(this);
	  },
	  render: function () {
	    return React.createElement(
	      'div',
	      { className: 'row margin-bottom-sm', 'data-type': 'item', 'data-idx': this.props.idx },
	      React.createElement('i', { className: 'fa fa-ellipsis-v dragable-icon ItemHandle', ref: 'ItemHandle' }),
	      React.createElement(
	        'div',
	        { className: 'col-6' },
	        React.createElement('input', { type: 'text', placeholder: 'Max 25 Characters', onChange: this.onChange("title"),
	          value: this.state.title, maxLength: '25' }),
	        this.props.shouldValidate ? React.createElement(Validation, { source: this.state.title, type: 'required' }) : null
	      ),
	      React.createElement(
	        'div',
	        { className: 'col-6' },
	        React.createElement(
	          'div',
	          { className: this.props.contentSize === 1 ? null : "margin-right-lg" },
	          React.createElement('input', { type: 'text', placeholder: 'http://link', onChange: this.onChange("link"), value: this.state.link }),
	          this.props.shouldValidate ? React.createElement(Validation, { source: this.state.link, type: 'link' }) : null
	        ),
	        this.props.contentSize === 1 ? null : React.createElement('i', { className: 'fa fa-trash fa-lg trash-icon', onClick: this.deleteItem })
	      )
	    );
	  }
	});
	...
	var AddLink = React.createClass({
	  ...
	  render: function () {
	    ...
	    if (this.props.items && this.props.items.length >= 8) msg = React.createElement(
	      'span',
	      { className: 'hint' },
	      '(You have added the maximum number of items.)'
	    );else msg = React.createElement(
	      'span',
	      { className: 'hint' },
	    ...
	    return React.createElement(
	      ...
	      React.createElement(
	        'a',
	        { className: this.props.items.length >= 8 ? "add-button disabled" : "add-button", href: '#',
	...
	var ResourceList = React.createClass({
	  ...
	  mixins: [sortable.ItemMixin, sortable.ListMixin],
	  itemOptions: {
	    handle: "ListHandle",
	    placeholder: "dashed"
	  },
	  ...
	  addList: function () {
	    var newId = 0;
	    this.state.content.map(function (obj) {
	      newId |= obj.id;
	    });
	    newId = (newId | newId + 1) - newId;

	    this.state.content.push({ "id": newId, "link": "", "title": "" });
	    this.props.handleChange('content', this.state.content, this.props.idx);
	    this.props.disableValidation();
	  },
	  handleDelete: function (idx) {
	    this.state.content.splice(idx, 1);
	    this.props.handleChange('content', this.state.content, this.props.idx);
	  },

	  onChange: function (key, value, itemIdx) {
	    return function (e) {
	      this.props.handleChange(key, value || e.target.value, this.props.idx, itemIdx);
	    }.bind(this);
	  },

	  swapItem: function (sourceIdx, targetIdx) {
	    var tmp = this.state.content[sourceIdx];
	    this.state.content.splice(sourceIdx, 1);
	    this.state.content.splice(targetIdx, 0, tmp);

	    this.props.handleChange('content', this.state.content, this.props.idx);
	  },
	  componentWillUpdate: function (nextProps, nextState) {
	    nextState.title = nextProps.list.title;
	    nextState.content = nextProps.list.content;
	  },

	  render: function () {

	    var draggable = this.props.listSize > 1;

	    var idxStyle = "list-idx";
	    var listStyle = "item-container list bg-color-gray margin-top-md";
	    if (draggable) {
	      idxStyle += " ListHandle";
	      listStyle += " move";
	    }
	    ...
	    return React.createElement(
	      'div',
	      { className: listStyle, 'data-type': 'list', 'data-idx': this.props.idx },
	      React.createElement(
	        'div',
	        { className: idxStyle, style: { "display": draggable ? "block" : "none" }, ref: 'ListHandle' },
	        React.createElement(
	          'span',
	          { className: 'idx' },
	          this.props.idx + 1
	      ...
	      React.createElement(
	        'div',
	        { className: 'control' },
	        this.props.draftIdx != this.props.idx ? React.createElement('span', { className: 'fa fa-pencil-square-o edit', onClick: this.props.openList(this.props.idx) }) : null,
	        this.props.listSize > 1 ? React.createElement('span', { className: 'fa fa-times', onClick: this.props.removeList(this.props.idx) }) : null
	      ),
	      this.props.draftIdx == this.props.idx ? React.createElement(
	        'div',
	        null,
	        React.createElement(
	          'h2',
	          { className: 'title margin-top-md' },
	          'Title of List',
	          React.createElement('span', { className: 'required-mark' })
	        ),
	        React.createElement('input', { type: 'text', placeholder: 'Max 30 Characters', onChange: this.onChange("title"), value: this.state.title,
	          maxLength: '30' }),
	        this.props.shouldValidate ? React.createElement(Validation, { source: this.state.title, type: 'required' }) : null,
	        React.createElement(
	          'div',
	        ...
	        React.createElement(
	          'div',
	          null,
	          this.state.content.map(function (item, idx) {
	            return React.createElement(ResourceListItem, _extends({ item: item,
	              key: item.id,
	              idx: idx,
	              index: idx,
	              swapItem: this.swapItem,
	              handleChange: this.onChange,
	              openList: this.props.openList(this.props.idx),
	              deleteItem: this.handleDelete,
	              contentSize: this.state.content.length,
	              shouldValidate: this.props.shouldValidate,
	              draggable: draggable
	            }, this.movableProps));
	          }.bind(this))
	        ),
	        React.createElement(AddLink, { addList: this.addList, items: this.state.content })
	      ) : React.createElement(
	        'div',
	        { className: 'row margin-top-lg list-content' },
	        React.createElement(
	          'h2',
	          { className: 'list-title margin-bottom-md' },
	          this.state.title
	        ),
	        React.createElement(
	          'ul',
	...
	var AddResourceList = React.createClass({
	  displayName: 'AddResourceList',

	  render: function () {
	    return React.createElement(
	      'div',
	      { className: 'item-container bg-color-gray margin-top-md' },
	      React.createElement(
	        'div',
	        { className: 'margin-top-md margin-bottom-md' },
	        React.createElement(
	          'a',
	          { className: 'add-button', href: '#', onClick: this.props.addList },
	          '+ Add another list'
	        ),
	        React.createElement(
	          'span',
	...
	var MainConfig = React.createClass({
	  ...
	  getInitialState: function () {
	    if (this.props.config["lists"] && this.props.config["lists"].length > 0) {
	      var obj = this.props.config;
	      obj["shouldValidate"] = false;
	      obj["draftIdx"] = -1;
	      return obj;
	    } else {
	      return {
	        "lists": [this.getEmptyListObj()],
	        "draftIdx": 0,
	        "shouldValidate": false
	      };
	    }
	  },
	  getEmptyListObj: function () {
	    return {
	      "id": 1,
	      "title": "",
	      "content": [{ "id": 1, "link": "", "title": "" }]
	    };
	  },
	  onChange: function (key, value, listId, itemId) {
	    if (typeof itemId !== "undefined") {
	      this.state.lists[listId].content[itemId][key] = value;
	    } else {
	      this.state.lists[listId][key] = value;
	    }
	    this.setState({ "lists": this.state.lists });
	    this.forceUpdate();
	  },
	  removeList: function (listIdx) {
	    return function () {
	      this.state.lists.splice(listIdx, 1);
	      if (listIdx < this.state.draftIdx) {
	        this.state.draftIdx -= 1;
	      } else if (listIdx == this.state.draftIdx) {
	        this.state.draftIdx = -1;
	      }
	  ...
	  prependHttpPrefix: function () {
	    var httpPreReg = new RegExp("^http(s)?://(.)+");
	    this.state.lists.map(function (list) {
	      list.content.map(function (item) {
	        item.title = item.title.trim();
	        item.link = item.link.trim();
	        if (!httpPreReg.test(item.link)) {
	          item["link"] = "http://" + item["link"];
	        }
	      });
	    });
	  },
	  validateList: function (listIdx) {
	    if (listIdx == -1) return true;

	    var list = this.state.lists[listIdx];
	    ...
	    for (var idx = 0; idx < items.length; idx++) {
	      ...
	      if (item.link.trim().length <= 0 || !validateLink(item.link)) return false;
	    }

	    return true;
	  },
	  onApply: function () {
	    if (this.validateList(this.state.draftIdx)) {
	      this.prependHttpPrefix();
	      jive.tile.close({ "data": { "lists": this.state.lists } });
	    } else {
	      this.setState({ "shouldValidate": true });
	    }
	  },
	  ...
	  addResourceList: function () {
	    ...
	    if (!this.validateList(this.state.draftIdx)) {
	      ...
	      this.state.draftIdx = this.state.lists.length;
	      this.state.shouldValidate = false;
	    }

	    var obj = this.getEmptyListObj();
	  ...
	  swapItem: function (sourceIdx, targetIdx) {
	    ...
	    var temp = this.state.lists[sourceIdx];
	    this.state.lists.splice(sourceIdx, 1);
	    this.state.lists.splice(targetIdx, 0, temp);
	    this.state.draftIdx = -1;
	    this.setState(this.state);
	  },
	  render: function () {
	    return React.createElement(
	      ...
	      React.createElement(
	        ...
	        React.createElement(
	          ...
	          'Display important links and resources in a tabular format'
	        )
	      ),
	      React.createElement(
	        'div',
	        ...
	        this.state.lists.map(function (list, idx) {
	          return React.createElement(ResourceList, _extends({ list: list,
	            idx: idx,
	            key: list.id,
	            index: idx,
	            draftIdx: this.state.draftIdx,
	            isMoveable: this.validateList(this.state.draftIdx),
	            listSize: this.state.lists.length,
	            swapItem: this.swapItem,
	      ...
	      this.state.lists.length > 3 ? null : React.createElement(AddResourceList, { addList: this.addResourceList }),
	      React.createElement(
	        'div',
	        { className: 'margin-top-lg' },
	        React.createElement(
	          'a',
	          { href: '#', className: 'btn btn-primary', onClick: this.onApply },
	          'Apply'
	        ),
	        React.createElement(
	          'a',
	          { href: '#', className: 'btn btn-secondary margin-left-md', onClick: this.onCancel },
	          'Cancel'
	        )
	      )
	    );
	...
	var migration = function (config) {
	  return {
	    "lists": config.lists.map(function (resourceList) {
	      var content = resourceList.content.map(function (item) {
	        return { "id": item.order, "title": item.title, "link": item.link };
	      });
	      return { "id": resourceList.order, "title": resourceList.title, "content": content };
	    })
	  };
	};
	...
	var initReact = function (config) {
	  // migration code
	  // remove order from previous config
	  if (config.lists && config.lists.length > 0 && config.lists[0].hasOwnProperty("order")) {
	    config = migration(config);
	  }

	  ReactDOM.render(React.createElement(MainConfig, { "config": config }), document.getElementById('app'));
	};
	...
	var fakeConfig = {
	  "lists": [{
	    "content": [{
	      "id": 1,
	      "link": "http://11.com/test/test/test/test/test/test/test/test/test/test/test/test/test",
	      "order": 1,
	      "title": "title 1",
	      "type": "Link"
	    }, {
